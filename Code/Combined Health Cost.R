...
pollutants <- read_csv("Combined Concentrations by Census Tract.csv")
...
# Step 2: Summarize total cost by pollutant and condition
cost_summary <- cases_with_cost %>%
  group_by(pollutant, condition) %>% # Group by 'condition'
  summarise(
    total_cost = sum(total_cost, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  arrange(pollutant, condition) # Arrange by 'condition'

# View the final result
print(cost_summary)

# Optionally, save the final results to a CSV file
write_csv(incidence_with_risk, "Combined_Cases_by_Tract.csv")
...
condition_order <- c("Typical Controlled", "Typical Uncontrolled")
...
gtsave(gt_table, "Combined Damages.docx", path = table_path)
...
#Economic costs by county in Maryland, Typical Uncontrolled####
incidence_with_risk_coi <- incidence_with_risk %>%
  left_join(costofillness, by = c("endpoint" = "Health Endpoint"))
...
ggplot(md_dc_map_data) +
  ...
  scale_fill_viridis_c(
    option = "plasma",
    na.value = "grey90",
    name = "Economic Cost ($Millions)",
    limits = c(0, 40), # Custom scale from 0 to 1.3 (million) - values above 1.3 will be capped to the max color
    oob = scales::squish, # Cap values outside range
    trans = "sqrt" # Apply square root transformation for skewing
  ) +
...
#Baltimore census tracts damages, Typical Uncontrolled####
...
ggplot(baltimore_map_data) +
  ...
  scale_fill_viridis_c(
    ...
    na.value = "grey90",
    name = "Economic Cost ($Millions)",
    limits = c(0, 1.3), # Custom scale from 0 to 1.3 (million) - values above 1.3 will be capped to the max color
    oob = scales::squish, # Cap values outside range
    trans = "sqrt" # Apply square root transformation for skewing
  ) +
  labs(title = "Curtis Bay Health-Related Economic Damages by Census Tract, Typical Uncontrolled",
