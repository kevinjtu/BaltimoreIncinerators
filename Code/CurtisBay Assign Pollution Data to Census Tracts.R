...
#
# Logic for each file:
# 1. Extracts condition and pollutant name from the filename.
...
input_directory <- "/Users/kevintu/Library/CloudStorage/OneDrive-UniversityofMarylandSchoolofMedicine/Climate Health Internship/AERMOD View Modeling Results/BenMAP Reformatted/CurtisBay"

# Set the desired path and name for the final output file.
output_file_path <- file.path(dirname(input_directory), "CurtisBay Concentrations by Census Tract.csv")
...
for (file_path in files_to_process) {
  ...
  # Extract condition and pollutant from the new filename format
  # e.g., "reformatted_Curtis Typical Controlled SO2 Annual.csv"
  match_groups <- str_match(file_name, "reformatted_Curtis\\s(.*?)\\s(\\S+)\\sAnnual\\.csv")
  condition <- match_groups[, 2] # e.g., "Typical Controlled"
  pollutant <- match_groups[, 3] # e.g., "SO2"
  
  pollution_df <- read_csv(file_path, show_col_types = FALSE) %>%
    select(latitude = Latitude, longitude = Longitude, Conc = Values) %>%
    mutate(
      pollutant = pollutant,
      condition = condition # Use 'condition' instead of 'year'
    )
...
final_combined_data_for_export <- final_combined_data %>%
  ...
  select(
    ...
    STATEFP,
    pollutant,
    condition, # Use 'condition' instead of 'year'
    Final_Conc,
    Source
  )
